...
function filterlist(selectobj) {
    ...
    // You can set the hook variable to a function that is called whenever the select list is filtered.
    // For example:
    ...
    this.init = function () {
        ...
        if (this.selectobj && this.selectobj.options) {
            for (var i = 0; i < this.selectobj.options.length; i++) {
                ...
                // If the value wasn't set in the original select list, then use the text.
        ...
        // Remember the selected option's value (if any option was selected by the user)
        if (selectobj.selectedIndex > -1) {
            this.selected_value = selectobj.options[selectobj.selectedIndex].value;
        }
    }
    ...
    this.set = function (pattern) {
        ...
        for (loop = 0; loop < this.optionscopy.length; loop++) {
            ...
            if ((this.match_text && regexp.test(option.text)) ||
                ...
                // We have a match, so add this option to the select list and increment the index
                this.selectobj.options[index++] = new Option(option.text, option.value, false);

                // If this currently processed option was selected beforehand then make it selected again after the filtering
                if (option.value == this.selected_value) {
                    this.selectobj.options[index-1].selected = "selected";
                }
